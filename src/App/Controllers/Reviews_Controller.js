...
class ReviewsController {
  async GetAllReviews(req, res, next) {
    ...
    try {
      ...
      const totalRatings = totalReview.reduce(
        (acc, review) => {
      ...
      await Tour.UpdateTourTotalRating(db, new ObjectId(id), {
        totalReview: roundedRating,
      });

      const combinedResults = await Promise.all(
        AllReviews.map(async (review) => {
          const commentsForReview = AllComments.filter(
            (comment) => comment.idRating.toString() === review._id.toString()
          );
          const firstComment =
            commentsForReview.length > 0 ? commentsForReview[0] : {};
          ...
          return {
            ...
            content:
              commentsForReview.length > 0
                ? censorReviewRegex(commentsForReview[0].content)
                : null,
    ...
    try {
      Connection.connect().then(async (db) => {
        ...
        if (
          ...
          return res
            .status(400)
            .send({ message: "You need to fill in complete information" });
        }
  ...
  async remoteComment(req, res, next) {
    ...
    try {
      const db = await Connection.connect();
      const commentsDetails = await Comments.remoteComment(db, id);
      return res
        .status(200)
        .send({ message: "Delete Success", data: commentsDetails });
    } catch (error) {
      console.error("Error fetching comments:", error);
      res
        .status(500)
